...
impl Pool where {
    ...
    pub fn execute<F>(&self, f:F) where F: FnMut(i64) + 'static + Send + Sync
    {
        ...
        let job = Message::NewJob(Arc::new(Mutex::new(f)));
...
fn test1() -> Job {
    ...
    let job = Arc::new(Mutex::new(move |val: i64| {instance.struct_function(val)}));
    ...
    c(123);
    */
    let cb = events_map.get(&123);
    //let mut c = cb.unwrap().lock().unwrap();
    //c(123);
    cb.unwrap().clone()
}
